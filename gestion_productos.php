    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto bg-surface-container-low p-6 md:p-12">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="bg-white border-4 border-on-background brand-shadow overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                ...
                                <th class="p-4 border-r-4 border-on-background">Imagen</th>
                                <th class="p-4 border-r-4 border-on-background">Nombre</th>
                                <th class="p-4 border-r-4 border-on-background">Marca</th>
                                <th class="p-4 border-r-4 border-on-background">Categoría</th>
                                <th class="p-4 border-r-4 border-on-background">Precio</th>
                                <th class="p-4 border-r-4 border-on-background">Stock</th>
                        ...
                        <tbody class="font-body-md text-on-background">
                            ...
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                $img_front = !empty($row['imagen_frontal']) ? 'uploads/' . $row['imagen_frontal'] : 'https://placehold.co/100x100/FFE7D0/1B1B1B?text=IMG';
                                ...
                                echo "<td class='p-4 border-r-4 border-on-background'><img src='" . $img_front . "' class='w-16 h-16 object-cover border-4 border-on-background brand-shadow'></td>";
                                echo "<td class='p-4 border-r-4 border-on-background font-black uppercase italic'>" . htmlspecialchars($row['nombre']) . "</td>";
                                echo "<td class='p-4 border-r-4 border-on-background font-bold uppercase text-xs text-primary'>" . htmlspecialchars($row['marca_nombre'] ?? 'N/A') . "</td>";
                                echo "<td class='p-4 border-r-4 border-on-background'><span class='px-3 py-1 border-2 border-on-background bg-secondary-container text-[10px] font-black uppercase'>" . htmlspecialchars($row['categoria_nombre']) . "</span></td>";
                                echo "<td class='p-4 border-r-4 border-on-background font-bold'>$" . number_format($row['precio'], 2) . "</td>";
                                echo "<td class='p-4 border-r-4 border-on-background font-bold'>" . $row['stock'] . "</td>";
                                echo "<td class='p-4'>
                        <div class='flex gap-4 justify-center'>
                            <a href='nuevo_producto.php?edit=" . $row['id'] . "' class='px-4 py-2 bg-primary text-on-primary font-black uppercase border-2 border-on-background brand-shadow btn-active hover:bg-primary-container transition-colors text-sm'>Editar</a>
                            <a href='controllers/AdminController.php?action=delete&id=" . $row['id'] . "' class='px-4 py-2 bg-error text-on-error font-black uppercase border-2 border-on-background brand-shadow btn-active hover:bg-red-700 transition-colors text-sm' onclick='return confirm(\"¿Estás seguro?\")'>Eliminar</a>
                        </div>
                      </td>";
                                echo "</tr>";
                            }
