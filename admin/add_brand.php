...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        $check_products = $pdo->prepare("SELECT COUNT(*) FROM productos WHERE marca_id = ?");
    ...
    if (!empty($nombre)) {
        try {
            if ($id) {
                ...
                $check = $pdo->prepare("SELECT id FROM marca WHERE nombre = ? AND id != ?");
                $check->execute([$nombre, $id]);
                if ($check->rowCount() > 0) {
                    $error = 'Ya existe otra marca con este nombre.';
                } else {
                    $stmt = $pdo->prepare("UPDATE marca SET nombre = ? WHERE id = ?");
                    $stmt->execute([$nombre, $id]);
                    $success = 'Marca actualizada con éxito.';
                }
            } else {
                // Modo Creación
                // Verificar si ya existe para evitar duplicados exactos
                $check = $pdo->prepare("SELECT id FROM marca WHERE nombre = ?");
                ...
                if ($check->rowCount() > 0) {
                    ...
                    $stmt->execute([$nombre]);
                    $success = 'Marca añadida con éxito.';
                }
            }
        } catch (Exception $e) {
        ...
        $error = 'Por favor, ingresa el nombre de la marca.';
    }
}
...
// Obtener todas las marcas para listarlas en la tabla
$marcas = [];
try {
    $stmt = $pdo->query("SELECT * FROM marca ORDER BY nombre ASC");
    $marcas = $stmt->fetchAll();
} catch (Exception $e) {
    $error = 'Error al cargar las marcas: ' . $e->getMessage();
}
?>
...
<head>
    ...
    <title>Marcas - Admin Dulcería</title>
    ...
    <div class="admin-container">
        <aside class="admin-sidebar">
            ...
            <nav class="admin-nav">
                <ul>
                    ...
                    <li><a href="add_brand.php" class="active"><i class="fa-solid fa-copyright"></i> Marcas</a></li>
        ...
        <main class="admin-main">
            <header class="admin-header">
                <h1><?php echo $edit_item ? 'Editar Marca' : 'Marcas'; ?></h1>
            ...
            <div class="admin-grid">
                ...
                <div class="admin-card">
                    <h2 class="table-title"><?php echo $edit_item ? '<i class="fa-solid fa-pen-to-square"></i> Editar Marca' : '<i class="fa-solid fa-plus"></i> Nueva Marca'; ?></h2>
                    ...
                    <form action="add_brand.php" method="POST" class="admin-form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="nombre" name="nombre" placeholder="Ej. Ricolino, De La Rosa" required value="<?php echo htmlspecialchars($edit_item['nombre'] ?? ''); ?>">
                        </div>

                        <?php if ($edit_item): ?>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 15px;">
                                <button type="submit" class="btn btn-primary" style="width: 100%;"><i class="fa-solid fa-save"></i> Guardar</button>
                                <a href="add_brand.php" class="btn-cancel"><i class="fa-solid fa-xmark"></i> Cancelar</a>
                            </div>
                        <?php else: ?>
                            <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 15px;"><i class="fa-solid fa-save"></i> Guardar Marca</button>
                        <?php endif; ?>
                    </form>
                </div>
                ...
                <div class="admin-table-card">
                    <h2 class="table-title">
                        ...
                        <span><?php echo count($marcas); ?></span>
                    </h2>
                    ...
                    <div class="table-responsive">
                        <table class="admin-table">
                            ...
                            <tbody>
                                <?php if (empty($marcas)): ?>
                                    <tr>
                                        <td colspan="3" style="text-align: center; padding: 40px; color: #7f8c8d;">No hay marcas registradas.</td>
                                    </tr>
                                <?php else: ?>
