...
<body class="bg-background text-on-background min-h-screen flex flex-col">
    ...
    <header
        ...
        <div class="flex items-center gap-4">
            <a href="index.php"
                class="p-2 border-2 border-on-background bg-secondary-fixed hover:bg-secondary-fixed-dim transition-all text-on-background"
            ...
            <img alt="Avatar de Administrador" class="w-10 h-10 border-2 border-on-background"
            ...
            <div class="max-w-5xl mx-auto">
                <div class="flex items-center gap-4 mb-10">
                    ...
                    <h1 class="font-headline-md text-headline-md text-on-background uppercase tracking-tighter italic">
                        <?php echo $is_edit ? 'Editar Dulce' : 'Añadir Nuevo Dulce'; ?>
                    </h1>
                </div>
                <form method="post" action="controllers/AdminController.php" enctype="multipart/form-data"
                    ...
                    <div class="lg:col-span-7 space-y-8">
                        <div class="bg-white border-4 border-on-background p-8 brand-shadow relative">
                            <div
                                ...
                                <span class="material-symbols-outlined text-3xl" data-icon="cake">cake</span>
                            </div>
                            <div class="space-y-6">
                                <div>
                                    <label
                                    ...
                                    <input name="nombre"
                                        ...
                                        placeholder="Ej. Caramelos de Selva Loca" type="text" />
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label
                                            class="block font-label-bold text-label-bold mb-2 text-on-background uppercase tracking-tighter">Stock</label>
                                        <input name="stock"
                                            value="<?php echo $is_edit ? htmlspecialchars($producto->stock) : ''; ?>"
                                            required
                                            class="w-full px-4 py-4 bg-surface-container-low border-2 border-on-background font-body-md focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all placeholder:text-on-background/40"
                                    ...
                                    <div>
                                        <label
                                            class="block font-label-bold text-label-bold mb-2 text-on-background uppercase tracking-tighter">Precio
                                            ($)</label>
                                        <input name="precio"
                                            value="<?php echo $is_edit ? htmlspecialchars($producto->precio) : ''; ?>"
                                            step="0.01" required
                                            class="w-full px-4 py-4 bg-surface-container-low border-2 border-on-background font-body-md focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all placeholder:text-on-background/40"
                                            placeholder="Ej. 15.50" type="number" />
                                    </div>
                                    <div>
                                        <label
                                            class="block font-label-bold text-label-bold mb-2 text-on-background uppercase tracking-tighter">Categoría</label>
                                        <div class="relative">
                                            <select name="categoria_id" required
                                                class="w-full px-4 py-4 bg-surface-container-low border-2 border-on-background font-body-md focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all appearance-none">
                                                <option value="">Seleccionar Categoría</option>
                                                <?php
                                                $stmtCat = $categoria->readAll();
                                                while ($rowCat = $stmtCat->fetch(PDO::FETCH_ASSOC)) {
                                                    $selected = ($is_edit && $producto->categoria_id == $rowCat['id']) ? 'selected' : '';
                                                    echo '<option value="' . $rowCat['id'] . '" ' . $selected . '>' . strtoupper(htmlspecialchars($rowCat['nombre'])) . '</option>';
                                                }
                                                ?>
                                            </select>
                                            <span
                                                class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">expand_more</span>
                                        </div>
                                    </div>
                                    <div>
                                        <label
                                            class="block font-label-bold text-label-bold mb-2 text-on-background uppercase tracking-tighter">Marca</label>
                                        <div class="relative">
                                            <select name="marca_id" required
                                                class="w-full px-4 py-4 bg-surface-container-low border-2 border-on-background font-body-md focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all appearance-none">
                                                <option value="">Seleccionar Marca</option>
                                                <?php
                                                $stmtMar = $marca->readAll();
                                                while ($rowMar = $stmtMar->fetch(PDO::FETCH_ASSOC)) {
                                                    $selected = ($is_edit && $producto->marca_id == $rowMar['id']) ? 'selected' : '';
                                                    echo '<option value="' . $rowMar['id'] . '" ' . $selected . '>' . strtoupper(htmlspecialchars($rowMar['nombre'])) . '</option>';
                                                }
                                                ?>
                                            </select>
                                            <span
                                ...
                                <div>
                                    <label
                                        class="block font-label-bold text-label-bold mb-2 text-on-background uppercase tracking-tighter">Descripción
                                        del Producto</label>
                                    <textarea name="descripcion" required
                                        class="w-full px-4 py-4 bg-surface-container-low border-2 border-on-background font-body-md focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all placeholder:text-on-background/40"
                                        placeholder="Describe el sabor, la textura y el empaque..."
                                        rows="5"><?php echo $is_edit ? htmlspecialchars($producto->descripcion) : ''; ?></textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="lg:col-span-5 space-y-8">
                        <div class="bg-surface-container-high border-4 border-on-background p-6 brand-shadow">
                            <label
                                class="block font-label-bold text-label-bold mb-4 text-on-background uppercase text-center tracking-widest italic">Imagen
                                del Producto (Frontal)</label>
                            <div class="relative">
                                <input type="file" name="imagen_frontal" accept="image/*" class="w-full p-2 mb-2" <?php echo !$is_edit ? 'required' : ''; ?> />
                                <?php if ($is_edit && !empty($producto->imagen_frontal)): ?>
                                    <div class="mt-2 text-sm text-center">
                                        <strong>Actual:</strong> <?php echo htmlspecialchars($producto->imagen_frontal); ?>
                                    </div>
                                <?php endif; ?>
                            </div>

                            <label
                                class="block font-label-bold text-label-bold mt-6 mb-4 text-on-background uppercase text-center tracking-widest italic">Imagen
                                Trasera (Opcional)</label>
                            <div class="relative">
                                <input type="file" name="imagen_trasera" accept="image/*" class="w-full p-2 mb-2" />
                                <?php if ($is_edit && !empty($producto->imagen_trasera)): ?>
                                    <div class="mt-2 text-sm text-center">
                                        <strong>Actual:</strong> <?php echo htmlspecialchars($producto->imagen_trasera); ?>
                                    </div>
                                <?php endif; ?>
                            </div>

                        </div>
                        <div class="bg-secondary-container border-4 border-on-background p-6 brand-shadow">
                            <h3
                                class="font-label-bold text-label-bold text-on-background uppercase flex items-center gap-2 mb-3 italic">
                                <span class="material-symbols-outlined" data-icon="lightbulb">lightbulb</span>
                                Tip de Inventario
                            </h3>
                            <p class="font-body-md text-on-background/80 leading-relaxed font-medium">
                                Usa fondos neutros para resaltar los colores. Las sombras suaves ayudan a que el
                                producto se vea más real y apetecible.
                            </p>
                        </div>
                    </div>
                </form>
